(function () {
    ...
    var common = require('./common.js'),
        ...
        examInput = [[409, 194, 207, 470, 178, 454, 235, 333, 511, 103, 474, 293, 525, 372, 408, 428],
        ...
        getSpreadsheetChecksum = function (spreadsheet) {
            ...
            for (i = 0; i < spreadsheet.length; i += 1) {
                for (j = 0; j < spreadsheet[i].length; j += 1) {
                    if (spreadsheet[i][j] > highNumber) {
                        highNumber = spreadsheet[i][j];
                    }
                    if (spreadsheet[i][j] < lowNumber || lowNumber === 0) {
                        lowNumber = spreadsheet[i][j];
                    }
                }
    ...
    examResult = getSpreadsheetChecksum(examInput);
